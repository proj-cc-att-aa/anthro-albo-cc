...
add_period <- function(df) {
  df %>%
    mutate(
      period = case_when(              ## will assign a period column to future_126, future_370 and future_585 depending upon which period they belong
        year >= 2023 & year <= 2040 ~ "2023–2040",
        year >= 2051 & year <= 2070 ~ "2051–2070",         ## we can take any period so later we took three final periods (one for near future and one for far future)
        year >= 2081 & year <= 2100 ~ "2081–2100",
        TRUE ~ NA_character_
      ),
      #period = factor(period, levels = c("2051–2060", "2091–2100"))
      period = factor(period, levels = c("2023–2040", "2051–2070", "2081–2100"))      ## periods 
    ) %>%
    filter(!is.na(period))
}
...
## combined maps
#scenarios <- c("SSP370")
# Our scenarios + periods (must match exactly what's in the data created above)
scenarios <- c("SSP370")            ## for multiple scenarios -  c("SSP126", "SSP370", "SSP585")
scenarios <- c("SSP126", "SSP370", "SSP585")
periods   <- c("2051–2070", "2081–2100")           
...
# Making plots for multiple scenarios/periods (example)
scenarios <- c("SSP370")                 # or c("SSP126","SSP370","SSP585")
scenarios <- c("SSP126", "SSP370", "SSP585")
periods   <- c("2051–2070", "2081–2100") # must match exactly (em-dash)
...
## Plotting the figure 2 again but with a big common legend for all the maps (update during revision)
...
#### journal style plot function without individual legends
plot_future_pfact_period_allregions_sci <- function(
    spatial_future_period_all_p,
    period_label,
    scenario_label = "SSP126",
    nuts3_map,
    p_limits = c(0, 1),
  ...
  # join onto geometry (plot all regions)
  map_P <- nuts3_map %>%
    left_join(
      p_one,
      by = "geo"
  ...
  # padded bbox so all panels align
  bb  <- sf::st_bbox(
    nuts3_map
  )
  dx  <- as.numeric(
  ...
  ## padding the xlimits and y limits
  xlim_pad <- c(
    bb["xmin"] -
      pad * dx,
  ...
  ggplot() +
    ...
    # background countries (ultra-thin lines)
    geom_sf(
      data = World_adm0,
      fill = rgb(
        215/255,
    ...
    # probability map (no internal borders)
    geom_sf(
      data = map_P,
      aes(
        fill = p_fact_mean
    ...
    # fixed 0–1 probability scale being retained across all maps while removing individual legends
    scale_fill_distiller(
      palette = pal_bwr,
      direction = -1,                     
      limits = p_limits,
    ...
    theme(
      ...
      # removing the individual legend from every map
      legend.position =
        "none",
...
# Make plots for multiple scenarios/periods
scenarios <- c(
  "SSP126",
  "SSP370",
  "SSP585"
...
for (sc in scenarios) {
  for (pr in periods) {
    ...
    future_plots_sci[[key]] <-
      plot_future_pfact_period_allregions_sci(
        spatial_future_period_all_p =
          spatial_future_period_all_p,
...
# Creating one common legend for us to use across all future maps
common_P_legend_plot <-
  ggplot(
    data.frame(
      x = 1,
      y = seq(
  ...
  # using exactly the same probability scale as our individual future maps
  scale_fill_distiller(
    palette = "RdBu",
    ...
    guide = guide_colorbar(
      ...
      ## Legend setting as per journal
      # IMPORTANT: making it a smooth continuous bar (not discrete boxes)
      direction =
        "vertical",
      # creating our common vertical colour bar at 150 mm height and 4 mm width for the complete plot
      barheight =
        unit(
          150,
          "mm"
  ...
  theme(
    ...
    # keeping our common legend title readable after removing legends from individual maps
    legend.title =
      element_text(
        size = 7,
        face = "bold",
    ...
    # keeping our common legend labels readable for the complete figure
    legend.text =
      element_text(
        size = 7,
        margin = margin(
...
# Extracting only our common legend for later figure assembly
common_P_legend <-
  cowplot::get_legend(
    common_P_legend_plot
  )
...
# Checking our common legend
cowplot::ggdraw(
  common_P_legend
)


# Saving the revised Figure 2 directory (TIFF + SVG)
## 1st the mapplot without figures and then the common legend
out_dir_save <-
  ".../VectorNet_ 2_original/Plots/A_final_plots/Main_article_plot/Plots_updated_after_2nd_revision/Figure_2_plots"
...
for (nm in names(future_plots_sci)) {
  ...
  # SVG (same size and specifications)
  ggplot2::ggsave(
    filename =
      file.path(
        out_dir_sci,
...
# Saving our single common probability legend separately for the complete figure
common_P_legend_to_save <-
  cowplot::ggdraw() +
  cowplot::draw_grob(
    common_P_legend,
    x = 0,
...
# Saving our common probability legend as TIFF
ggplot2::ggsave(
  filename =
    file.path(
      out_dir_sci,
      "Pfact_future_common_legend.tiff"
    ),
  ...
  # allowing some additional width from usual width of legend defined above for legend labels and heading
  width =
    1.6,
  height =
    16.0,
...
# Saving our common probability legend as SVG
ggplot2::ggsave(
  filename =
    file.path(
      out_dir_sci,
      "Pfact_future_common_legend.svg"
    ),
  ...
  # allowing us some additional width for legend labels and heading
  width =
    1.6,
  height =
    16.0,
...
###############################################!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!######################
## Figure 3 (A-C) for article - Maps and plots for attribution
###############################################!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!######################
...
## Plotting the figure 3(A-C) again but with a big common legend for all the maps (update during revision)

# Common legend scale across ALL future maps (all SSPs + all periods)
af_limits <- range(
  spatial_future_period_all$AF_abs_pct_mean,
  na.rm = TRUE
)


# breaks (we also force 0 to be included)
af_breaks <- pretty(
  spatial_future_period_all$AF_abs_pct_mean,
  6
)
...
#### Journal-style plot function without individual legends
plot_future_AF_period_allregions_sci <- function(
    spatial_future_period_all,
    period_label,
    scenario_label = "SSP126",
    nuts3_map,
    af_limits,
  ...
  ggplot() +
    # background countries (ultra-thin lines like journal guidelines)
    geom_sf(
      data  = World_adm0,
      fill  = rgb(
        215/255,
    ...
    # AF% map (no internal borders)
    geom_sf(
      data  = map_AF,
      aes(
        fill = AF_abs_pct_mean
      ),
      color = NA,
      linewidth = 0
    ) +
    
    # common AF% scale being retained across all maps while removing individual legends
    scale_fill_distiller(
      palette   = pal_af,
      direction = 1,
      limits    = af_limits,
...
for (sc in scenarios) {
  ...
  for (pr in periods) {
    ...
    future_plots_AF_sci[[key]] <-
      plot_future_AF_period_allregions_sci(
        spatial_future_period_all =
          spatial_future_period_all,
...
common_AF_legend_plot <-
  ...
  # using exactly the same AF scale as our individual future maps
  scale_fill_distiller(
    palette   = "YlOrRd",
    direction = 1,
    limits    = af_limits,
    ...
    guide = guide_colorbar(
      ...
      # creating our common vertical colour bar at 150 mm height and 10 mm width (for the complete plot)
      barheight =
        unit(
          150,
          "mm"
      ...
      ticks.linewidth =     ## width of half tick lines denoting lables on legend
        0.15,
...
# Saving to revised Figure 3 (A-C) directory (TIFF + SVG)
## 1st the mapplot without figures and then the common legend 
out_dir_save <-
  ".../VectorNet_ 2_original/Plots/A_final_plots/Main_article_plot/Plots_updated_after_2nd_revision/Figure_4_plots"
...
# Saving our single common AF legend separately for the complete figure
common_AF_legend_to_save <-
  cowplot::ggdraw() +
...
# Saving our common AF legend as TIFF
ggplot2::ggsave(
  filename =
    file.path(
      out_dir_sci,
      "AF_future_common_legend.tiff"
    ),
...
# Saving our common AF legend as SVG
ggplot2::ggsave(
  filename =
    file.path(
      out_dir_sci,
      "AF_future_common_legend.svg"
    ),
...
###############################################!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!######################
## Figure 3 (Panel D) - future attributed population at risk
###############################################!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!######################

#Helper functions 
## .RData files are being loaded into separate environments
load_rdata_to_env <- function(path) {
  e <- new.env(parent = emptyenv())
  nms <- load(path, envir = e)
  list(env = e, names = nms)
...
## function to pick up the posterior prediction sample dataframe from the loaded file 
pick_pred_df <- function(e) {
  ...
  dfs <- objs[
    vapply(
      objs,
      function(nm) inherits(e[[nm]], c("data.frame", "tbl_df", "tbl")),
      logical(1)
    )
  ]
  
  if (length(dfs) == 0)
    stop("No data.frame/tibble found in .RData")
  
  ## scores are assigned based on what columns a dataframe contains inside pi_env$env
  score_df <- function(df) {
    ...
    10L * ("geo"  %in% nm) +
      10L * ("draw" %in% nm) +
      5L * any(c("year", "year_idx") %in% nm) +
      5L * any(c("p", "p_pred", "prob", "p_mean") %in% nm)
  }
  
  scored <- tibble(
    obj = dfs,
    score = vapply(dfs, \(nm) score_df(e[[nm]]), integer(1))
  ) %>%
    arrange(desc(score))
  ...
  if (
    !("geo" %in% names(df)) ||
    !("draw" %in% names(df)) ||
    !any(c("year", "year_idx") %in% names(df))
  ) {
    stop("Could not detect posterior prediction dataframe.")
  }
...
## function to pick up the posterior probability column
pick_prob_col <- function(df) {
  ...
  stop("Could not find posterior probability column.")
}


## SSP and corresponding piControl predictions are being joined
make_future_scenario <- function(base_dir, ssp_code) {
  
  #ssp_code = "370"
  
  file_pi  <- file.path(base_dir, sprintf(
    "future_prediction_picontrol_climate_ssp%s_socioeconomic_2022_2080_output.Rdata", ssp_code    ## .Rdata file name does not reflect the years it contain, all files have data from 2022-2100
  ))
  ...
  df_pi  <- pick_pred_df(pi_env$env)     ## will give the dataframe inside pi_env$env which contain predicted posteriors samples i.e. in our case (p_post_pred_all)
  df_ssp <- pick_pred_df(ssp_env$env)
  
  # standardize keys
  key_cols <- intersect(names(df_pi), names(df_ssp))    
  
  ## in our case year and year_idx column is present
  if (!("year" %in% key_cols) && "year_idx" %in% key_cols) {    

    df_pi  <- df_pi  %>% mutate(year = ifelse("year" %in% names(df_pi), year, 2021 + year_idx))    ## if no year column  then start the year from 2022 as our prediction start from 2022
    df_ssp <- df_ssp %>% mutate(year = ifelse("year" %in% names(df_ssp), year, 2021 + year_idx))
    key_cols <- union(setdiff(key_cols, "year_idx"), "year")
  }
  
  join_keys <- intersect(c("geo","draw","year"), key_cols)     ## take the joining keys we have (since we are aggregating spatially then it must have geo, draw (will aggregte later for all draws) and year)
  if (length(join_keys) < 3) stop("Join keys missing. Need geo, draw, year in both future dfs.")
  
  # picking probability column from  each (picontrol and ssp's) posterior sample dataframe
  pcol_pi  <- pick_prob_col(df_pi)        
  pcol_ssp <- pick_prob_col(df_ssp)
  
  ## joining both the dataset  with geo, year and draw (like factual and counterfactual) and replace the p names win both dataset with factual (p_fact) and counterfactual (p_cf) names
  df_post_fact_cf <- df_ssp %>%
    transmute(geo, draw, year, p_fact = .data[[pcol_ssp]]) %>%
    left_join(
      df_pi %>% transmute(geo, draw, year, p_cf = .data[[pcol_pi]]),
...
base_dir <- paste0(
  ".../",
  "VectorNet_ 2_original/Output/A-Final_output/Main_model_output"
)

## construction of future scenario datasets 
future_126 <- make_future_scenario(base_dir, "126")
future_370 <- make_future_scenario(base_dir, "370")
future_585 <- make_future_scenario(base_dir, "585")
...
## population objects are being extracted from .RData files
load_population_object <- function(rdata_path) {
  obj_names <- load(rdata_path)
  if ("population" %in% obj_names) return(get("population"))
  get(obj_names[1])
...
# load population tables (one per scenario) 
pop_126 <- load_population_object("Data/climate/Final_covariates_data/Future_data/Final_future_ssp126_population_data_yearly.RData")
pop_370 <- load_population_object("Data/climate/Final_covariates_data/Future_data/Final_future_ssp370_population_data_yearly.RData")
pop_585 <- load_population_object("Data/climate/Final_covariates_data/Future_data/Final_future_ssp585_population_data_yearly.RData")
...
# Attributable population at risk 
## attributable population iscalculated for each posterior draw
compute_future_pop_risk_prob <- function(future_df,
                                         pop_df,
                                         scenario_label,
                                         positive_only = TRUE,
  ...
  # Join population onto each posterior row (replicated over draws)
  DT <- DT_fut[DT_pop, nomatch = 0L]  # keeps only rows with population
  ...
  # Option: keeping attribution prob in [0,1] (usually safe)
  if (clamp_01) {
    DT[, delta_p := pmin(pmax(delta_p, 0), 1)]
  }
  ...
  # Sum across regions for each draw-year
  pop_risk_draw_year <- DT[, .(
    pop_at_risk = sum(pop_contrib, na.rm = TRUE)
  ), by = .(draw, year)]
  
  # Posterior summary by year
  pop_risk_post <- pop_risk_draw_year[, .(
    pop_mean = mean(pop_at_risk, na.rm = TRUE),
    pop_lwr  = quantile(pop_at_risk, 0.025, na.rm = TRUE),
    pop_upr  = quantile(pop_at_risk, 0.975, na.rm = TRUE)
...
# Combine for plotting (bind rowise)
pop_risk_future_all <- rbindlist(list(pop_risk_126, pop_risk_370, pop_risk_585), use.names = TRUE, fill = TRUE)

# Keep scenario order consistent in legend (already consitent in our case, just a double check step)
pop_risk_future_all[, scenario := factor(scenario, levels = c("SSP126","SSP370","SSP585"))]


## results from 2023 onward were being retained
pop_risk_future_all_2023 <- pop_risk_future_all[
  year >= 2023
]
...
theme_science_small <- function(legend_pos = c(0.90, 0.38)) {
  ggplot2::theme_classic() +
    ggplot2::theme(
      ...
      # axis typography + thin lines (journal like)
      axis.title = element_text(size = 7),
      axis.text  = element_text(size = 6, color = "black"),
      axis.line  = element_line(linewidth = 0.2),
      axis.ticks = element_line(linewidth = 0.2,color = "black"),
...
p_pop_risk_future_sci <- ggplot(
  ...
  scale_x_continuous(
    limits =  x_limits,
    breaks = seq(floor(x_limits[1]/10)*10, ceiling(x_limits[2]/10)*10, by = 10),
    expand = expansion(mult = c(0, 0.01))   # small right padding
  ) + 
  scale_y_continuous(
    breaks = c(40, 80, 120, 160, 200, 240) * 1e6,
    labels = scales::label_number(scale = 1e-6, suffix = "M", accuracy = 1),
  ...
  theme_science_small(legend_pos = c(0.12, 0.90)) +    ## topleft corner
  guides(
    color = guide_legend(ncol = 1),
    fill  = guide_legend(ncol = 1)
  )
...
###############################################!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!######################
## Fig S19. (addition during revision) - Sensitivity plot for article (sensitivity analysis of attributable pop at risk with scenario simulations)
###############################################!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!######################

## Helper functions
# Loading files into a separate environment
load_rdata_to_env <- function(path) {
  e <- new.env(parent = emptyenv())
  nms <- load(path, envir = e)
  list(env = e, names = nms)
...
# Selecting the posterior prediction dataframe
pick_pred_df <- function(e) {
  #e = pi_env$env
  objs <- ls(e)
  dfs  <- objs[vapply(objs, function(nm) inherits(e[[nm]], c("data.frame","tbl_df","tbl")), logical(1))]
  ...
  # Scoring candidate dataframes to identify posterior predictions
  score_df <- function(df) {
    nm <- names(df)
    score <- 0L
    score <- score + ifelse("geo"  %in% nm, 10L, 0L)
  ...
  # Selecting the highest-scoring dataframe
  df <- e[[scored$obj[1]]]
...
# Selecting the posterior probability column
pick_prob_col <- function(df) {
  #df <- df_pi
  nm <- names(df)
  # Preferring the draw-level probability column
  candidates <- c("p", "p_pred", "prob", "p_mean")
  col <- candidates[candidates %in% nm][1]
  if (!is.na(col)) return(col)
...
# Defining the sensitivity-analysis prediction directory
sens_dir <- paste0(
  ".../",
  "VectorNet_ 2_original/Output/A-Final_output/Main_model_output/",
  "Sensitivity_analysis_result_(2nd_revision)"
)


# Finding the required sensitivity prediction file
find_sensitivity_file <- function(
    base_dir,
    climate_type,
    donor_ssp,
    sensitivity_type
  ...
  # Identifying factual SSP3-7.0 or piControl climate files
  if (climate_type == "ssp370") {
  ...
  } else if (climate_type == "picontrol") {
    ...
    stop(
      "climate_type must be either 'ssp370' or 'picontrol'"
    )
  }
  
  
  # Identifying the SSP supplying the swapped socioeconomic trajectory
  donor_match <- grepl(
    paste0(
      "SSP",
      donor_ssp
  ...
  # Identifying the sensitivity component
  if (sensitivity_type == "pop_mobility") {
    sens_match <- grepl(
      "pop_dens_and_mobility",
      file_names,
      ignore.case = TRUE
    )
    
  } else if (sensitivity_type == "urban_land") {
    sens_match <- grepl(
      "urban_land",
      file_names,
      ignore.case = TRUE
  ...
  } else if (sensitivity_type == "full_socio") {
    sens_match <- grepl(
      "full_socio",
      file_names,
      ignore.case = TRUE
  ...
  # Checking that one matching file is available
...
# Locating all sensitivity files

# Population density + mobility
# SSP1-derived
fact_popmob_126_file <- find_sensitivity_file(
  sens_dir,
  climate_type = "ssp370",
  donor_ssp = "126",
  sensitivity_type = "pop_mobility"
)


pi_popmob_126_file <- find_sensitivity_file(
  sens_dir,
  climate_type = "picontrol",
  donor_ssp = "126",
  sensitivity_type = "pop_mobility"
)

# SSP5-derived
fact_popmob_585_file <- find_sensitivity_file(
  sens_dir,
  climate_type = "ssp370",
  donor_ssp = "585",
  sensitivity_type = "pop_mobility"
...
pi_popmob_585_file <- find_sensitivity_file(
  sens_dir,
  climate_type = "picontrol",
  donor_ssp = "585",
  sensitivity_type = "pop_mobility"
...
# Urban land
# SSP1-derived
fact_urban_126_file <- find_sensitivity_file(
  sens_dir,
  climate_type = "ssp370",
  donor_ssp = "126",
  sensitivity_type = "urban_land"
)


pi_urban_126_file <- find_sensitivity_file(
  sens_dir,
  climate_type = "picontrol",
  donor_ssp = "126",
  sensitivity_type = "urban_land"
...
# SSP5-derived
fact_urban_585_file <- find_sensitivity_file(
  sens_dir,
  climate_type = "ssp370",
  donor_ssp = "585",
  sensitivity_type = "urban_land"
...
pi_urban_585_file <- find_sensitivity_file(
  sens_dir,
  climate_type = "picontrol",
  donor_ssp = "585",
  sensitivity_type = "urban_land"
...
# Full socioeconomic conditions
# SSP1-derived
fact_full_126_file <- find_sensitivity_file(
  sens_dir,
  climate_type = "ssp370",
  donor_ssp = "126",
  sensitivity_type = "full_socio"
)


pi_full_126_file <- find_sensitivity_file(
  sens_dir,
  climate_type = "picontrol",
  donor_ssp = "126",
  sensitivity_type = "full_socio"
...
# SSP5-derived
fact_full_585_file <- find_sensitivity_file(
  sens_dir,
  climate_type = "ssp370",
  donor_ssp = "585",
  sensitivity_type = "full_socio"
...
pi_full_585_file <- find_sensitivity_file(
  sens_dir,
  climate_type = "picontrol",
  donor_ssp = "585",
  sensitivity_type = "full_socio"
...
c(
  fact_popmob_126_file,
  pi_popmob_126_file,
  fact_popmob_585_file,
  pi_popmob_585_file,
  fact_urban_126_file,
  pi_urban_126_file,
  fact_urban_585_file,
...
# Creating matched factual and counterfactual sensitivity datasets
# We compare SSP3-7.0 climate with piControl under the same socioeconomic condition
make_future_sensitivity_pair <- function(
    factual_file,
    picontrol_file
) {
  
  # factual_file = fact_popmob_126_file
  # picontrol_file = pi_popmob_126_file
  
  # Loading both prediction files
  fact_env <- load_rdata_to_env(
    factual_file
  )
  
  pi_env <- load_rdata_to_env(
    picontrol_file
  )
  
  # Selecting posterior draw-level predictions
  df_fact <- pick_pred_df(
    fact_env$env
  )
  ...
  # Creating calendar year if only year_idx is available
  if (!("year" %in% names(df_fact))) {
    if (!("year_idx" %in% names(df_fact))) {
      stop("Neither year nor year_idx found in factual sensitivity file.")
    }
    
    df_fact <- df_fact %>%
      mutate(
        year = 2021L + as.integer(year_idx)
      )
  }
  
  # Repeating the year assignment for piControl
  if (!("year" %in% names(df_pi))) {
    if (!("year_idx" %in% names(df_pi))) {
      stop("Neither year nor year_idx found in piControl sensitivity file.")
    }
  ...
  # Selecting factual and counterfactual probability columns
  pcol_fact <- pick_prob_col(
    df_fact
  )
  pcol_pi <- pick_prob_col(
    df_pi
  )
  
  
  # Keeping variables required for attribution
  df_fact_small <- df_fact %>%
    transmute(
      geo = as.character(geo),
      draw = as.integer(draw),
      year = as.integer(year),
  ...
  # Joining factual and counterfactual posterior draws
  df_post_fact_cf <- df_fact_small %>%
    left_join(
      df_pi_small,
      by = c(
        "geo",
  ...
  # Checking that all factual predictions have matching piControl predictions
  if (anyNA(df_post_fact_cf$p_cf)) {
    stop(
      "Some factual rows did not receive matching piControl predictions."
    )
  }
...
# Creating matched factual-counterfactual sensitivity datasets
# Population density + mobility
# SSP1-derived
future_popmob_126 <- make_future_sensitivity_pair(
  factual_file = fact_popmob_126_file,
  picontrol_file = pi_popmob_126_file
)
...
# SSP5-derived
future_popmob_585 <- make_future_sensitivity_pair(
  factual_file = fact_popmob_585_file,
  picontrol_file = pi_popmob_585_file
)


# Urban land
future_urban_126 <- make_future_sensitivity_pair(
  factual_file = fact_urban_126_file,
  picontrol_file = pi_urban_126_file
)

future_urban_585 <- make_future_sensitivity_pair(
  factual_file = fact_urban_585_file,
  picontrol_file = pi_urban_585_file
)


# Full socioeconomic conditions
future_full_126 <- make_future_sensitivity_pair(
  factual_file = fact_full_126_file,
  picontrol_file = pi_full_126_file
)

future_full_585 <- make_future_sensitivity_pair(
  factual_file = fact_full_585_file,
  picontrol_file = pi_full_585_file
)



# Computing expected attributable population at risk
compute_future_pop_risk_prob <- function(future_df,
                                         pop_df,
                                         scenario_label,
                                         positive_only = TRUE,
  ...
  # Joining population to posterior predictions
  DT <- DT_fut[DT_pop, nomatch = 0L]
  
  # Calculating attributable probability
  DT[, delta_p := p_fact - p_cf]
  ...
  # Keeping attribution within 0-1
  if (clamp_01) {
    DT[, delta_p := pmin(pmax(delta_p, 0), 1)]
  }
  ...
  # Summing across regions for each draw-year
  pop_risk_draw_year <- DT[, .(
    pop_at_risk = sum(pop_contrib, na.rm = TRUE)
  ), by = .(draw, year)]
  
  # Summarizing across posterior draws
  pop_risk_post <- pop_risk_draw_year[, .(
    pop_mean = mean(pop_at_risk, na.rm = TRUE),
    pop_lwr  = quantile(pop_at_risk, 0.025, na.rm = TRUE),
    pop_upr  = quantile(pop_at_risk, 0.975, na.rm = TRUE)
...
# We keep SSP370 absolute population fixed for population-at-risk weighting while socioeconomic covariates vary inside the establishment model
# Reducing each large posterior object to a yearly summary before removing it

# SSP3 reference
# Computing our common SSP3 reference once
pop_risk_att_reference_370 <- compute_future_pop_risk_prob(
  future_370,
  pop_370,
  "SSP3 reference",
  positive_only = TRUE,
...
# Removing the large posterior object after summarizing
rm(future_370)
gc()



# Population density + mobility sensitivity
# Using SSP370 absolute population for weighting in all trajectories
# SSP1-derived population density + mobility
pop_risk_att_popmob_126 <- compute_future_pop_risk_prob(
  future_popmob_126,
  pop_370,
  "SSP1-derived",
  positive_only = TRUE,
...
# Removing the large posterior object
rm(future_popmob_126)
gc()


# Reusing our SSP3 reference
pop_risk_att_popmob_370 <- copy(
  pop_risk_att_reference_370
)


# SSP5-derived population density + mobility
pop_risk_att_popmob_585 <- compute_future_pop_risk_prob(
  future_popmob_585,
  pop_370,
  "SSP5-derived",
  positive_only = TRUE,
...
# Removing the large posterior object
rm(future_popmob_585)
gc()




# Combining population density + mobility trajectories
pop_risk_att_popmob_all <- rbindlist(
  list(
    pop_risk_att_popmob_126,
    pop_risk_att_popmob_370,
    pop_risk_att_popmob_585
  ),
...
# Keeping projections from 2023 onward
pop_risk_att_popmob_all_2023 <-
  pop_risk_att_popmob_all[
...
# Keeping scenario order consistent
pop_risk_att_popmob_all_2023[
  ,
  scenario := factor(
    scenario,
    ...
    # Using labels describing the substituted conditions
    labels = c(
      "SSP1 population + mobility",
      "SSP3 reference",
      "SSP5 population + mobility"
    )
  )
]
...
# Removing intermediate summary objects
rm(
  pop_risk_att_popmob_126,
  pop_risk_att_popmob_370,
  pop_risk_att_popmob_585,
...
# Urban-land sensitivity
# Varying urban land while keeping SSP370 population weighting fixed
# SSP1-derived urban land
pop_risk_att_urban_126 <- compute_future_pop_risk_prob(
  future_urban_126,
  pop_370,
  "SSP1-derived",
  positive_only = TRUE,
...
# Reusing our SSP3 reference
pop_risk_att_urban_370 <- copy(
  pop_risk_att_reference_370
)

# SSP5-derived urban land
pop_risk_att_urban_585 <- compute_future_pop_risk_prob(
  future_urban_585,
  pop_370,
  "SSP5-derived",
  positive_only = TRUE,
...
# Combining urban-land trajectories
pop_risk_att_urban_all <- rbindlist(
  list(
    pop_risk_att_urban_126,
    pop_risk_att_urban_370,
    pop_risk_att_urban_585
  ),
  use.names = TRUE,
  fill = TRUE
)

pop_risk_att_urban_all_2023 <-
  pop_risk_att_urban_all[
    year >= 2023 &
      year <= 2100
  ]
...
# Full socioeconomic sensitivity
# Varying all SSP-dependent socioeconomic covariates while keeping SSP370 population weighting fixed

# SSP1-derived full socioeconomic conditions
pop_risk_att_full_126 <- compute_future_pop_risk_prob(
  future_full_126,
  pop_370,
  "SSP1-derived",
  positive_only = TRUE,
...
# Reusing our SSP3 reference
pop_risk_att_full_370 <- copy(
  pop_risk_att_reference_370
)


# SSP5-derived full socioeconomic conditions
pop_risk_att_full_585 <- compute_future_pop_risk_prob(
  future_full_585,
  pop_370,
  "SSP5-derived",
  positive_only = TRUE,
...
# Combining full socioeconomic trajectories
pop_risk_att_full_all <- rbindlist(
  list(
    pop_risk_att_full_126,
    pop_risk_att_full_370,
    pop_risk_att_full_585
  ),
  use.names = TRUE,
  fill = TRUE
)

pop_risk_att_full_all_2023 <-
  pop_risk_att_full_all[
    year >= 2023 &
      year <= 2100
  ]
...
pop_risk_att_full_all_2023[
  ...
  scenario := factor(
    ...
    # Using labels describing the substituted conditions
    labels = c(
      "SSP1 socio-environmental",
      "SSP3 reference",
      "SSP5 socio-environmental"
    )
  )
]
...
# Keeping our final yearly sensitivity summaries
pop_risk_att_popmob_all_2023
pop_risk_att_urban_all_2023
pop_risk_att_full_all_2023

# Removing the common SSP3 reference after constructing all sensitivity datasets
rm(pop_risk_att_reference_370)

gc()



# Plotting the sensitivity analyses
# Using a compact journal-style theme
theme_science_small <- function(
    legend_pos = c(0.90, 0.38)
) {
  ggplot2::theme_classic() +
    ggplot2::theme(
      text = element_text(
...
# Using distinct colours for the sensitivity trajectories
# Climate remains SSP3-7.0 in all factual runs
sens_cols <- c(
  "SSP1 population + mobility" = "darkorchid4",
  "SSP3 reference" = "darkgreen",
  "SSP5 population + mobility" = "chocolate3",
  "SSP1 urban land" = "darkorchid4",
  "SSP5 urban land" = "chocolate3",
  "SSP1 socio-environmental" = "darkorchid4",
  "SSP5 socio-environmental" = "chocolate3"
)


# Plotting attributable population at risk across sensitivity conditions
plot_attributable_sensitivity <- function(
    plot_data,
    legend_pos = c(0.14, 0.90)
) {
  
  # Defining x-axis limits and breaks
  x_limits <- range(
    plot_data$year,
    na.rm = TRUE
  )
  ...
  ggplot(
    ...
    # Showing 95% posterior credible intervals
    geom_ribbon(
      aes(
        ymin = pop_lwr,
        ymax = pop_upr,
...
# Plotting population density + mobility sensitivity
p_att_popmob <- plot_attributable_sensitivity(
  pop_risk_att_popmob_all_2023,
  legend_pos = c(
    0.17,
    0.90
...
# Plotting urban-land sensitivity
p_att_urban <- plot_attributable_sensitivity(
  pop_risk_att_urban_all_2023,
  legend_pos = c(
    0.17,
    0.90
...
# Plotting full socioeconomic sensitivity
p_att_full <- plot_attributable_sensitivity(
  pop_risk_att_full_all_2023,
  legend_pos = c(
    0.17,
    0.90
...
out_dir_save <- paste0(
  '.../VectorNet_ 2_original/Plots/A_final_plots/attribution_sensitivity_analysis(2nd_revision)'
)

out_dir_sens_att <- file.path(
  out_dir_save,
  "Attribution_sensitivity_socio-environmental"
)
...
# Saving population density + mobility sensitivity
# TIFF
ggplot2::ggsave(
  filename = file.path(
    out_dir_sens_att,
    "Attributable_PAR_sensitivity_population_density_mobility.tiff"
  ),
  plot = p_att_popmob,
  device = ragg::agg_tiff,
  width = width_cm,
...
ggplot2::ggsave(
  filename = file.path(
    out_dir_sens_att,
    "Attributable_PAR_sensitivity_population_density_mobility.svg"
  ),
  plot = p_att_popmob,
  device = "svg",
  width = width_cm,
...
# Saving urban-land sensitivity
# TIFF
ggplot2::ggsave(
  filename = file.path(
    out_dir_sens_att,
    "Attributable_PAR_sensitivity_urban_land.tiff"
  ),
  plot = p_att_urban,
  device = ragg::agg_tiff,
  width = width_cm,
...
ggplot2::ggsave(
  filename = file.path(
    out_dir_sens_att,
    "Attributable_PAR_sensitivity_urban_land.svg"
  ),
  plot = p_att_urban,
  device = "svg",
  width = width_cm,
...
# Saving full socioeconomic sensitivity
# TIFF
ggplot2::ggsave(
  filename = file.path(
    out_dir_sens_att,
    "Attributable_PAR_sensitivity_full_socioeconomic.tiff"
  ),
  plot = p_att_full,
  device = ragg::agg_tiff,
  width = width_cm,
...
ggplot2::ggsave(
  filename = file.path(
    out_dir_sens_att,
    "Attributable_PAR_sensitivity_full_socioeconomic.svg"
  ),
  plot = p_att_full,
  device = "svg",
  width = width_cm,
...
########    Extracting sensitivity results for reporting     ###################
# Checking selected years
pop_risk_att_popmob_all_2023[
  year %in% c(
    2030,
    2050,
...
# Calculating our late-century mean for 2081-2100
late_popmob <- pop_risk_att_popmob_all_2023[
  year >= 2081 &
    year <= 2100,
  .(
    mean_PAR = mean(
...
late_urban <- pop_risk_att_urban_all_2023[
  year >= 2081 &
    year <= 2100,
  .(
    mean_PAR = mean(
...
late_full <- pop_risk_att_full_all_2023[
  year >= 2081 &
    year <= 2100,
  .(
    mean_PAR = mean(
...
p_hr_future_decade_sci <- ggplot(
  ...
  # Reference line: HR = 1 indicates equal outbreak hazard under factual and counterfactual no-climate-change conditions
  # Values > 1 indicate a higher outbreak hazard under the factual climate, whereas values < 1 indicate a lower outbreak hazard.
  geom_hline(
    yintercept = 1,
    linetype = "dashed",
    linewidth = 0.3,
  ...
  # Interannual 2.5–97.5% interval within each decade
  geom_errorbar(
  ...
  # Adding small left/right padding
  scale_x_discrete(
    labels = decade_labels,
    expand = expansion(add = c(0.4, 0.0))   
  ) +
  
  # Logarithmic y-axis because hazard ratios are multiplicative: 
  # HR = 2 and HR = 0.5 (are equally distant from the reference HR = 1 as log(0.5)= -0.693 = −log(2))
  scale_y_log10(
  ...
  labs(
    x = "Decade",
    y = "Factual-to-counterfactual outbreak hazard ratio"
  ) +
...
# Points show the mean area-weighted factual-to-counterfactual post-establishment outbreak hazard ratio within each decade.
# The horizontal dashed line at HR = 1 indicates equal outbreak hazard under factual and counterfactual no-climate-change conditions.
# Values > 1 indicate a higher outbreak hazard under the factual climate, whereas values < 1 indicate a lower outbreak hazard.
# Error bars show the 2.5–97.5% range across annual values within the period. We will not call these intervals as credible intervals
# The y-axis is shown on a logarithmic scale because hazard ratios are multiplicative quantities.
...
## ## Plotting the figure 6(A-C) again but with a big common legend for all the maps (update during revision)
## common function using one common legend across all scenarios & all periods
#### Journal-style plot function without individual legends
plot_future_deltaR_period_allregions_sci <- function(
    spatial_deltaR_period_all,
    period_label,
  ...
  # spatial_future_period_all = spatial_hr_period_all
  #period_label   = "2081–2100"
  # scenario_label = "SSP585"
  # nuts3_map      = nuts3_map     ## sf_object
  # delta_limits      = hr_limits
  # delta_breaks      = hr_breaks
  #pal_hr         = "YlOrBr"
  
  
  
  # filter scenario + period (scenario as well as entered period in function input)
  delta_one <- spatial_deltaR_period_all %>%
    filter(
      scenario == scenario_label,
      period == period_label
  ...
  # join onto map geometry (plot all regions)
  map_deltaR <- nuts3_map %>%
    left_join(
      delta_one,
      by = "geo"
    )
  ...
  ggplot() +
    ...
    # delta map without internal NUTS3 borders
    geom_sf(
      data  = map_deltaR,
      aes(
        fill = deltaR_period_mean
      ),
      color = NA,
      linewidth = 0
    ) +
    ...
    # common outbreak risk index scale being retained across all maps while removing individual legends
    scale_fill_distiller(
      palette = pal_delta,
      direction = 1,
      limits = delta_limits,       ## common legend across all time periods and scenarios
      breaks = delta_breaks,       ## common breaks across all time periods and scenarios
      labels = scales::label_number(
        accuracy = 1
      ),                            
      name = "Outbreak\nRisk Index",       
...
##PLOTTING with one common legend across all scenarios and all time period
## Common scale across all scenario-period maps (this will give common limits across all scenarios in all time period)
delta_limits <- range(
  spatial_deltaR_period_all$
    deltaR_period_mean,
  na.rm = TRUE
...
for (sc in scenarios) {
  for (pr in periods) {
    ...
    future_plots_deltaR_sci[[key]] <-
      plot_future_deltaR_period_allregions_sci(
        spatial_deltaR_period_all =
          spatial_deltaR_period_all,
        ...
        nuts3_map =
          nuts3_map,
        delta_limits =
          delta_limits,    ## common limit across all scenario all time period
        delta_breaks =
          delta_breaks,     ## common breaks across all scenario all time period
        pal_delta =
          "RdPu"          
      )
  }
...
common_deltaR_legend_plot <-
  ...
  geom_tile() +
  # using exactly the same outbreak risk index scale as our individual future maps
  scale_fill_distiller(
    palette =
      "RdPu",
    direction =
...
# Extracting only our common legend for later figure assembly  
common_deltaR_legend <-
  cowplot::get_legend(
    common_deltaR_legend_plot
  )
#!! Warnings! because there are 12 maplots and only any one is used to extract common legend
...
## SAVING all map plots with common limit of legend across all scenarios and all time periods 
## Saving to revised Figure 6 directory

## 1st the mapplot without figures and then the common legend
out_dir_deltaR_map_common <-
  ".../VectorNet_ 2_original/Plots/A_final_plots/Main_article_plot/Plots_updated_after_2nd_revision/Figure_6_plots"
...
# Saving our single common outbreak risk index legend separately for the complete figure
common_deltaR_legend_to_save <-
  cowplot::ggdraw() +
  cowplot::draw_grob(
    common_deltaR_legend,
    x = 0,
...
# Saving our common outbreak risk index legend as TIFF
ggplot2::ggsave(
  filename =
    file.path(
      out_dir_deltaR_map_common,
      
      "Outbreak_deltaR_future_common_legend.tiff"
    ),
...
# Saving our common outbreak risk index legend as SVG
ggplot2::ggsave(
  filename =
    file.path(
      out_dir_deltaR_map_common,
      
      "Outbreak_deltaR_future_common_legend.svg"
    ),
  plot =
    common_deltaR_legend_to_save,
  device =
...
# Note - he same plotting code as above is used to generate the upper and lower bounds of the 95% credible interval for the Integrated Outbreak Risk Index shown in Figs. S21 and S22. 
## The only change is that the upper and lower 95% credible interval estimates are used instead of the posterior mean when determining the legend range and generating the plots.
## Specifically, spatial_deltaR_period_all$deltaR_period_mean is replaced by spatial_deltaR_period_all$deltaR_period_upr for the upper 95% credible interval and by spatial_deltaR_period_all$deltaR_period_lwr for the lower 95% credible interval.
