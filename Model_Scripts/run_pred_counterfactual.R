...
n_years_pred <- length(unique(pred_data$year_idx)) # prediction_years i.e.  length of years of prediction data


# Run inla model for all years prediction in loop

########### Important-------------------
##For year 1 (2010) it uses the existing y_proximity_exp300 and bin_idx_y_proximity_exp300 in pred_data (which is same as proximity for training data for 2011, used to initalize the proximity for prediction)
...
#  check that prediction tibble must have a 'year' column
...
for (i in seq_along(years_pred)) {
  ...
  ##  Ensure proximity columns are set correctly
  if (i == 1) {
    # For the first prediction year (year_idx == 1) we use the proximity that is already present in pred_data (computed earlier from training data for 2010 and saved in y_proximity_exp300 + bin_idx_y_proximity_exp300). 
    pred_data_curr_effective <- pred_data_curr
...
# Save counterfactual prediction outputs (2011–2021)
...
pred_output_file <- file.path(
  full_model_dir,
  "counterfactual_prediction_2011_2021_output.RData"
)
