...
for(year in 2001:2023){
  ...
  nc_file_path <- paste0("Netcdf_file/20crv3-era5_obsclim_pr_mm_global_daily_", year, ".nc")
  ...
  #  load the NetCDF file as a raster for spatial extraction.
  rast_file <- rast(nc_file_path)
  print(rast_file)
  

  # loading our model shapefile and keep it in geographic coordinates.
  ...
  # compute cell areas so our polygon means are area-weighted.
  ...
  # building the output table with one row per region and one column per day.
