# ================== Future mean temperature NetCDF extraction to regional CSV files ==================
...
for(year in 2021:2080){             ### take care of time values for 2010, origin is 2001-01-01, for 2011 to 2020 (origin is 2011-01-01) and from 2021 to 2024 (origin is 2021-01-01)
  ...
  nc_file_path <- paste0("Netcdf_file/gfdl-esm4_ssp126_tas_celsius_global_daily_", year, ".nc")   ## for ssp370
...
# years we want to export (edit if needed)
years_ssp      <- 2015:2100
years_picontrol <- 1995:2100   # change to 1995:2100 if we really want the whole piControl window

# scenarios + models (as per the requirement )
scenarios <- c("Pi_control", "SSP1", "SSP3", "SSP5")
models    <- c("GFDL_model", "IPSL_model", "MPI_model", "MRI_model", "UKESM_model")

# variables we want to extract from NetCDF file names
jobs <- list(
  list(var = "tas",  out_subdir = "temperature_mean",   file_prefix = "temperature_mean")     ## only temperature extraction in this code (multiscenario _multimodel temperature extraction)
)
...
read_nc_var_meta <- function(nc_path, var) {
  ...
  units <- ncatt_get(nc, var, "units")$value      ## units 
  fill1 <- ncatt_get(nc, var, "_FillValue")$value    ## fill value for missing data
  fill2 <- ncatt_get(nc, var, "missing_value")$value  ### fill value for NA data

  fill <- NA_real_
  if (!is.null(fill1) && !is.na(fill1)) fill <- as.numeric(fill1)
  if (is.na(fill) && !is.null(fill2) && !is.na(fill2)) fill <- as.numeric(fill2)

  list(units = units, fill = fill)      ## take out the unit and fill value
}

# Find the correct NetCDF file inside Netcdf_file/ by variable token (_tas_, _pr_, _hurs_)
find_nc_for_var <- function(nc_dir, var) {

  candidates <- list.files(nc_dir, pattern = paste0("_", var, "_.*\\.nc$"), full.names = TRUE)    ## taking out the tas net cdf file inside correct directory for anyone scenario and anyone model
  if (length(candidates) == 0) {
    candidates <- list.files(nc_dir, pattern = "\\.nc$", full.names = TRUE)
  }
  if (length(candidates) == 0) return(NA_character_)
  candidates[1]
}

# Core extractor: one NetCDF (scenario×model×var) -> yearly CSVs
extract_one_nc_yearly <- function(nc_path, var, poly_sf, out_dir, years_keep, file_prefix) {

  dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

  dates <- read_nc_dates(nc_path, time_var = "time")      
  meta  <- read_nc_var_meta(nc_path, var)              ## reading metadata like units and fill value from function defined above 

  r <- terra::rast(nc_path)

  # set NAflag if the NetCDF has a fill value
  if (!is.na(meta$fill)) terra::NAflag(r) <- meta$fill    # (there should be a value very large or small i.e. fill value if not NA)

  # ensure CRS is defined
  if (is.na(terra::crs(r))) terra::crs(r) <- "EPSG:4326"

  # cell area weights (single-layer raster; used for all days)
  ...
  for (yr in years_keep) {
    ...
    message("✅ ", basename(nc_path), " | ", var, " | year ", yr, " saved -> ", out_file)
  }
}

# MAIN LOOP: ALL scenarios × models × vars

for (sc in scenarios) {

  years_keep <- if (sc == "Pi_control") years_picontrol else years_ssp  ## if picontrol then starts from 1995 otherwise starts from 2015 for ssp's
  ...
  for (md in models) {
    ...
    for (jb in jobs) {
      var        <- jb$var
      out_subdir <- jb$out_subdir
      prefix     <- jb$file_prefix

      nc_path <- find_nc_for_var(nc_dir, var)

      if (is.na(nc_path) || !file.exists(nc_path)) {
        message("⚠️ No NetCDF found for var=", var, " in: ", nc_dir)
        next
      }

      out_dir <- file.path(model_dir, out_subdir)

      message("\n--- Running: scenario=", sc, " | model=", md, " | var=", var, " ---")
      extract_one_nc_yearly(
        nc_path     = nc_path,
        var         = var,
        poly_sf     = poly_sf,
        out_dir     = out_dir,
