# ================== Future precipitation NetCDF extraction to regional CSV files ==================
...
for(year in 2021:2080){             ### take care of time values for 2010, origin is 2001-01-01, for 2011 to 2020 (origin is 2011-01-01) and from 2021 to 2024 (origin is 2021-01-01)
  ...
  nc_file_path <- paste0("Netcdf_file/gfdl-esm4_ssp126_pr_mm_global_daily_", year, ".nc")          ## for ssp370
  ...
  origin_date <- strsplit(origin_date, " ")[[1]][1]   # Extract the date 
  ...
  output_file <- paste0("precipitation/precipitation_mean_", year, ".csv")
...
# NOTE: folder inside each model_dir where we save CSVs = "precipitation"
jobs <- list(
  list(var = "pr", out_subdir = "precipitation", file_prefix = "precipitation_mean")
)
...
# Read variable unit and fill value (so we can convert + set NA flag robustly)
...
detect_precip_varname <- function(nc_path, preferred = "pr") {
...
## Find correct precip NetCDF inside Netcdf_file/
find_nc_for_pr <- function(nc_dir) {

  c1 <- list.files(nc_dir, pattern = "_pr_.*mmday.*\\.nc$", full.names = TRUE, ignore.case = TRUE)
  if (length(c1) > 0) return(c1[1])

  c2 <- list.files(nc_dir, pattern = "_pr_.*\\.nc$", full.names = TRUE, ignore.case = TRUE)
...
extract_pr_one_nc_yearly <- function(nc_path, poly_sf, out_dir, years_keep, file_prefix) {
  ...
  varname <- detect_precip_varname(nc_path, preferred = "pr")
  ...
  for (yr in years_keep) {
    ...
    # final df: Location + daily precip columns
    ...
    message("✅ ", basename(nc_path), " | pr | year ", yr, " saved -> ", out_file)
  }
}

# MAIN LOOP: ALL scenarios × models (PRECIP ONLY)
for (sc in scenarios) {
  ...
  for (md in models) {
    ...
    # find precip nc for this scenario × model
    nc_path <- find_nc_for_pr(nc_dir)

    if (is.na(nc_path) || !file.exists(nc_path)) {
      message("⚠️ No precipitation NetCDF found in: ", nc_dir)
      next
    }

    out_dir <- file.path(model_dir, "precipitation")

    message("\n--- Running precipitation: scenario=", sc, " | model=", md, " ---")
    extract_pr_one_nc_yearly(
      nc_path     = nc_path,
      poly_sf     = poly_sf,
      out_dir     = out_dir,
      years_keep  = years_keep,
      file_prefix = "precipitation_mean"
    )
  }
}

message("\n🎉 All precipitation extraction done.")
